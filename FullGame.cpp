...
int main(){
    ...
    char level;
    int level_int;
    cout << "Welcome to Matty and Sophia's Final Project for CSCI1300! \nOur game will start by choosing the difficulty level you want to play on,\n which will determine the amount of pirates included. Following this,\n you will be prompted by a merchant to purchase items with the coins you are given. \nThe items you can purchase are weapons to fight pirates, hints to solve riddles located in traps, \nand an extra-life if you fail to defeat a pirate or solve a riddle. After you have selected everything you desire from the merchant, \nyou will be prompted with the main menu, which consists of the options: move player, show inventory, display board, display leaderboard, and quit. \nThe main menu will continue to pop up throughout the game if you run out of moves, fight a pirate, or solve a riddle. \nYou will use a, s, w, or d to move left, down, up, or right throughout the game. \nThe map will explode to help you navigate throughout the game depending on if run into open space, a pirate, or a trap. \nTo win the game, you must collect three of the five keys in the game which will come from solving riddles. \nTry to avoid the pirates and answer riddles to the best of your ability. Good luck!" << endl;
    Map map;
    int rt;
    bool inLoop = true;
    cout << endl;
    ...
    while(inLoop){
        ...
        if(level == '1'){
            level_int = 1;
            rt = map.readAndAddTraps("riddles.txt");
            ...
            case '1':
                ...
                for(int i = 0; i < moves; i++){
                    ...
                    // Else statement to end the game
                        ...
                        map.updateLeaderboardFile("Leaderboard.txt");
                        p.endGame('w');
                    }
                    else{
                        p.endGame('l');
