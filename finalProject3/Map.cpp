...
void Map:: printLeaderboard(string filename){
    ...
    while(getline(fin, line)){
        if(line.length() > 0){
            ...
            Person temp_person(temp_arr[0], stoi(temp_arr[1]), stoi(temp_arr[2]));
            cout << (count+1) << ": " ;
            cout << leaderboard.at(count).getName() << " - " << leaderboard.at(count).getMoves() <<  " - level: " << leaderboard.at(count).getLevel() << endl;
            count++;
        }
    }
}
int Map::updateLeaderboardFile(string filename, Person new_person){
    ...
    while(getline(fin, line)){
        if(line.length() > 0){
            split(line, ',', temp_arr, 3);
            Person temp_person(temp_arr[0], stoi(temp_arr[1]), stoi(temp_arr[2]));
            addPerson(temp_person, leaderboard);
        }
    }
    for(int i = 0; i < leaderboard.size(); i++){
        fout << (i+1) << ": " ;
        fout << leaderboard.at(i).getName() << " - score: " << leaderboard.at(i).getMoves() << " - level: " << leaderboard.at(i).getLevel() << endl;
    }
    fout.close();
    return 0;
}
