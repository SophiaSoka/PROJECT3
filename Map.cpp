...
void Map:: printLeaderboard(){
    for(int i = 0; i < leaderboard.size(); i++){
        cout << (i+1) << ": " ;
        cout << leaderboard.at(i).getName() << " - " << leaderboard.at(i).getMoves() << endl;
    }
}
void Map::updateLeaderboardFile(string filename){
    for(int i = 0; i < leaderboard.size(); i++){
        cout << (i+1) << ": " ;
        cout << leaderboard.at(i).getName() << " - " << leaderboard.at(i).getMoves() << endl;
    }
}
bool Map::addPerson(Person new_person){
    int new_time = new_person.getMoves();
    int temp_time = 0;
    for(int i = 0; i < leaderboard.size(); i++){
        temp_time = leaderboard.at(i).getMoves();
        if(temp_time > new_time){
